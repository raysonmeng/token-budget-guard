...
- **Core** — Node implementation (usage probe, three-tier guard, blocking-MCP continuation) for both Claude Code and Codex. The only remaining shell script is the optional auto-resume `watchdog.sh`. ✅
...
- **Tests** — 208 (core) + 37 (Codex MCP/installer) passing.
...
- **核心** —— Node 实现(用量探针、三档守卫、阻塞 MCP 续接),Claude 与 Codex 共用;唯一保留的 shell 脚本是可选的自动续跑 `watchdog.sh`。✅
- **安装器** —— 经大量交叉审针对真实配置加固(幂等、字节级完美卸载、不破坏用户已有配置)。✅
- **分发** —— `npx agent-quota-guard …` 已就绪;**已发布到 npm**(v0.2.0)。
- **测试** —— 208(核心)+ 37(Codex MCP / 安装器)全通过。
